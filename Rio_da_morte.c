...
/*
P5) O problema dos 3 canibais e 3 missionários
...
Há uma outra dificuldade: em nenhum momento o número de canibais pode ser superior ao número de missionários 
pois desta forma os missionários estariam em grande perigo de vida. Como administrar a travessia?

Para resolver este problema, considere inicialmente as situações (ou estados) 
que podem ocorrer numa das margens no que se refere ao número de canibais e missionários. 
Podemos representar cada um destes estados por um par ordenado (c,m), 
onde c e m representam o número de canibais e de missionários, respectivamente. 
Desde que 0 =< c <= 3 e que 0 =< m <= 3, há, no total, 16 possíveis estados, que vão de (0,0) a (3,3).

Contudo, alguns destes estados não são viáveis em função da restrição imposta pelo problema 
de que em nenhum momento o número de canibais pode ser superior ao número de missionários. 
É o caso dos pares ordenados (2,1), (3,1) e (3,2), assim como dos pares (1,2), (0,2) e (0,1). 
Nestes últimos três casos, a restrição se verifica na margem oposta, 
já que os canibais e missionários restantes (complemento em relação a (3,3)) estão lá.
*/
typedef struct no{
    int canibais;
    int missio;
