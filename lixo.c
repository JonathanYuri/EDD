...
int Fluxo (int coord_semaforos[][2], char semaforo[][6], locura IClandia[][37], int diferenca[], int num_semaf)
{
    int x = coord_semaforos[num_semaf][0];
    int y = coord_semaforos[num_semaf][1];
    
    char verY = semaforo[num_semaf][2];
    char verX = semaforo[num_semaf][5];
    ...
    int quantV = contar_carros (verY, x, y, IClandia); //^^
    int quantH = contar_carros (verX, x, y, IClandia); // --->
...
int main()
{
    ...
    for (int j = 0; j < 96; j++)
    {
        mudanca = Fluxo (coord_semaforos, semaforo, IClandia, diferenca, j);
        if (mudanca != 0)
        {
            // eu so tirar ate r1
            ...
            if (diferenca[0] == 1)
            {
                printf("Vou mudar a vertical ");
            }
            else
            {
                printf("Vou mudar a horizontal ");
            }
            //printf("%i ", diferenca[0]);
            //printf("%i (%i)\n", diferenca[1], mudanca);
            
            //printf("ANTES DO CALCULO (%c %c), (%c %c)\n", semaforo[j][0], semaforo[j][1], semaforo[j][3], semaforo[j][4]);
            if ((mudanca +'0')< semaforo[j][1]) // 1  v2 r3 -> v1 r2
            {
                semaforo[j][1] = semaforo[j][1] - mudanca;
                semaforo[j][4] = semaforo[j][4] - mudanca;
                
                //printf("DEPOIS DO CALCULO (%c %c), (%c %c)\n", semaforo[j][0], semaforo[j][1], semaforo[j][3], semaforo[j][4]);
            }
            else  // se for igual ou maior eu vou colocar no amarelo
            {
                if (semaforo[j][0] == 'v')
                {
                    semaforo[j][0] = 'a';
